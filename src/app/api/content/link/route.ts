...
export async function POST(request: NextRequest) {
  try {
    ...
    let body: { url?: string; title?: string; mode?: 'single' | 'multiple' }
    try {
    ...
    const { url, title: userTitle, mode = 'single' } = body
    ...
    try {
      ...
      const { title: pageTitle, content: pageContent } = await fetchLinkContent(url.trim())

      const finalTitle = userTitle?.trim() || pageTitle || null
      const rawText = pageContent

      await db
        .update(contentInputs)
        .set({ rawText, title: finalTitle })
      ...
      if (mode === 'multiple') {
        ...
        const [draft] = await db
          ...
          .values({
            contentInputId: contentInput.id,
            userId: user.id,
            title: finalTitle ?? result.suggestedTitle,
      ...
      console.error('[/api/content/link]', error)
      const msg = error instanceof Error ? error.message : String(error)
      return NextResponse.json({ error: msg }, { status: 500 })
    }
  } catch (error) {
    console.error('[/api/content/link] Unhandled:', error)
